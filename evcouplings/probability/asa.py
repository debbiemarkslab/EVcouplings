...
def combine_asa(remapped_pdb_files, dssp_binary, prefix, outcfg):
    """
    Parameters
    ----------
    ...
    for file in remapped_pdb_files:
        if valid_file(file):
            ...
            file_path_prefix = file.rsplit(".pdb", maxsplit=1)[0]
            pdb_prefix = file_path_prefix.rsplit("/", maxsplit=1)[-1]

            dssp_output_file = prefix + pdb_prefix + ".dssp"
            rsa_output_file = prefix + pdb_prefix + "_rsa.csv"
