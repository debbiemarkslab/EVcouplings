...
def most_similar_by_organism(similarities, id_to_organism):
    ...
    for species in species_groups.groups.keys():
        ...
        # else if length is longer than 1, get the 
        # most similar
        else:
            max_identity = species_subset.identity_to_query.idxmax()
            id = species_subset.loc[max_identity,:].id
            identity_to_query = species_subset.loc[max_identity,:].identity_to_query
...
def filter_best_reciprocal(alignment, paralogs, most_similar_in_species, allowed_error=0.02):
    ...
    for index,row in most_similar_in_species.iterrows():
        ...
        # then that sequence is more similar to a paralog, and don't save it in the final
        # alignment
        if not np.any(identity_mat[:, alignment_index] > \
