...
def get_distance(annotation1, annotation2):
    ...
    Parameters
    ----------
    annotation1 : tuple of (int, int)
        genome start and end location for the first CDS

    annotation2 : tuple of (int, int)
        genome start and end location for the first CDS
    ...
    location_1 = sorted(
        (annotation1[0], annotation1[1])
    )

    location_2 = sorted(
        (annotation2[0], annotation2[1])
    )
...
def best_reciprocal_matching(possible_partners):
    ...
    possible_partners :  pd.DataFrame
        ...
        Gives all pairings of uniprot identifiers from alignment 1 
        with every uniprot identifier from alignment 2 that is found in 
        the same genome, and the number of
        nucleotides between their corresponding coding DNA sequences
    ...
    # look at all sequences in first alignment, and find the closest reciprocal
    # partner for each
        ...
        if genome in location_groups_2:
            ...
            for _,first_cds in gene_location_subset_1.iterrows():
                for _,second_cds in gene_location_subset_2.iterrows():
                    ...
                    uniprot_id_1 = first_cds["uniprot_ac"]
                    uniprot_id_2 = second_cds["uniprot_ac"]

                    possible_partners_list.append(
                        (uniprot_id_1, uniprot_id_2, distance_between_genes)
                    )

    possible_partners = pd.DataFrame(possible_partners_list,
        columns = ['uniprot_id_1','uniprot_id_2','distance']
    )
